...
「このごろ」は、株式会社MIXIが運営する家族アルバムアプリ「[みてね](https://mitene.us/)」の、写真を家族だけで共有して一緒に成長や日々を振り返る体験をリスペクトしている。

一方で「みてね」は子どもを中心に始めるサービスであり、子どものいない夫婦が旅行やお出かけ、何気ない毎日の写真・動画を家族へ共有する今回の用途では利用できなかった。そこで、子どものプロフィールではなく**夫婦のタイムラインから始まる家族アルバム**として「このごろ」を作った。
...
| role | 画面上の名称 | 主な想定 | 役割 |
|---|---|---|---|
| `owner` | 管理者 | 夫婦 | 投稿とイベントの管理に加え、家族の招待と権限変更を行う |
| `uploader` | 投稿者 | 投稿を任せる家族 | 投稿とイベントを管理するが、家族情報にはアクセスしない |
| `viewer` | 閲覧者 | 両親・兄弟など | 写真・動画の閲覧とコメントだけを行う |

| 操作 | `owner` | `uploader` | `viewer` |
|---|:---:|:---:|:---:|
| タイムライン・近況の閲覧 | 可 | 可 | 可 |
| イベント・投稿詳細の閲覧 | 可 | 可 | 可 |
| 写真・動画の表示と保存 | 可 | 可 | 可 |
...
| コメントの追加 | 可 | 可 | 可 |
| 自分のコメントの削除 | 可 | 可 | 可 |
| 他のメンバーのコメントの削除 | 可 | 不可 | 不可 |
| 写真・動画の投稿 | 可 | 可 | 不可 |
| 投稿の削除 | すべて | すべて | 不可 |
| イベントの作成・編集・削除 | 可 | 可 | 不可 |
| セクションの作成・編集・削除 | 可 | 可 | 不可 |
| イベントカバーの変更 | 可 | 可 | 不可 |
| 家族一覧とLINE連携状態の閲覧 | 可 | 不可 | 不可 |
| 招待URLの発行と招待時roleの指定 | 可 | 不可 | 不可 |
| 他のメンバーのrole変更 | 可 | 不可 | 不可 |
| 自分の表示名・LINE通知設定 | 可 | 可 | 可 |
| テーマ設定 | 可 | 可 | 可 |

`owner`は自分自身のroleを変更できない。`uploader`の投稿削除権限は自身の投稿だけでなく、家族内のすべての投稿を対象とする。`viewer`向けの管理導線は非表示にし、クライアントルーティングとAPI認可でもアクセスを拒否する。
