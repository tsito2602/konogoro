...
Post Detailは遷移元を背面に残した、ほぼ全画面のBottom Sheetとして表示する。Postを押したらデータ取得の完了を待たずにURLを切り替えてシートを下からスライドインさせ、Skeleton、読み込み結果、読み込みエラーをシート内へ描画する。シートの表示とデータ取得は並行し、シートを閉じたときは遷移元とそのスクロール位置を維持する。
