...
選択した写真はローカルのObject URLで即時表示し、動画は準備中であることが分かるプレースホルダーを即時表示する。端末内の準備処理は同時実行数を制限してUI操作とメモリを圧迫しないようにし、完了した項目からサムネイルへ切り替える。準備中とアップロード中はヘッダー直下に固定した進捗を表示し、個別の準備失敗は対象項目から再試行できるようにする。投稿編集で写真・動画を追加する場合も同じ挙動とする。
