...
- 設定画面でシステム・ライト・ダークをアイコン付きで選択できる。既定値はシステムとし、`prefers-color-scheme`による端末設定に従う。選択は端末内に保存する。
- ダークテーマでは背景・surface・区切り線・補助文字を暗色向けに切り替え、写真と動画を主役に保つ。
- `docs/DESIGN.md`の青いaccentは、このアプリではアプリアイコンの朝日`#FFD166`へ置き換える。
- ライトテーマのBottom Navigationは、選択中のアイコンと文字にゴールドイエロー`#967000`を使い、線と文字の太さでも未選択状態と区別する。ダークテーマでは朝日の黄色`#FFD166`を使う。
