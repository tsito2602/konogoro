...
PRを作成・更新したら、毎回、追加承認を求めず以下の統合・デプロイ成功確認まで行う。Draft、目視確認待ち、ローカルブラウザ不調でも省略しない。文書のみのPRも対象とする。権限不足・保護ルール・デプロイ失敗の場合は原因と未反映の状態を報告する。

## 複数PRをまとめて確認する

1. 現在の`staging`と確認中PRを確認する。最新`main`を起点に統合する場合も、確認中の既存PRを再統合して維持し、他の作業者の同時更新を上書きしない。
2. 確認中の既存PRと今回作成・更新したPRの最新commitを、依存関係の順に`staging`へmergeする。
