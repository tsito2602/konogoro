...
初回は上記を文書の地図として使い、各作業ではAGENTS.mdと変更に関係する節だけを読む。毎回すべてを読み直す必要はない。検証・レビューはAGENTS.md「差分に応じた検証」に一元化する。ユーザーの明示指示を優先し、各文書の役割を越えた矛盾は関連箇所を同時に修正する。機能要件中の古い配置文言を理由に、改訂した画面仕様を無効にしない。
