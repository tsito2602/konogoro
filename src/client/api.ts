...
export function eventDate(start: string | null, end: string | null): string {
  ...
  const startLabel = new Intl.DateTimeFormat("ja-JP", { year: "numeric", month: "short", day: "numeric" }).format(new Date(`${start}T00:00:00`));
  if (!end || end === start) return startLabel;
  const endLabel = new Intl.DateTimeFormat("ja-JP", { month: "short", day: "numeric" }).format(new Date(`${end}T00:00:00`));
