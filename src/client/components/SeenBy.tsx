...
export function SeenBy({ users }: { users: SeenUser[] }) {
  if (users.length === 0) return <span className="seen-by-empty" aria-label="見た人 0人"><SeenIcon /><span>0</span></span>;
  return <details className="seen-by">
    <summary aria-label={`見た人 ${users.length}人、一覧を表示`}><SeenIcon /><span>{users.length}</span></summary>
