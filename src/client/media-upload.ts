...
export function validateMediaFiles(files: File[], currentCount: number): string | null {
  const invalid = files.find((file) => !allowedTypes.has(file.type) || file.size > (file.type.startsWith("video/") ? 500 : 25) * 1024 * 1024);
...
export function uploadFile(url: string, body: Blob, contentType = body.type, onProgress?: (loaded: number) => void): Promise<void> {
