...
export function EventCreatePage() {
  ...
  return <>
    ...
    <main className="form-page page-content"><form onSubmit={submit} className="form-stack">
      <label>タイトル<input name="title" required maxLength={100} autoFocus /></label>
      <div className="date-row"><label>開始日<input name="startDate" type="date" /></label><label>終了日<input name="endDate" type="date" /></label></div>
      <label>メモ<textarea name="description" rows={4} maxLength={1000} /></label>
      {error && <p className="form-error" role="alert">{error}</p>}
      <button className="primary-button wide" disabled={saving}>{saving ? "作成中…" : "作成"}</button>
    </form></main>
  </>;
}
