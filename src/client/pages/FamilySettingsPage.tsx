...
export function FamilySettingsPage() {
  ...
  const removeMember = async () => {
    ...
    setRemoving(true); setMemberError("");
    try {
  ...
  return <>
    ...
    <main className="page-content family-page">
      {error ? <ErrorState message={error} retry={load} /> : members === null ? <Loading /> : <>
        <section className="family-section">
          <h2>権限設定</h2>
          {members.length === 0 ? <EmptyState title="メンバーはいません" body="招待URLを発行してメンバーを招待できます。" /> : <div className="member-list">
            {members.map((member) => <article className="member-row" key={member.id}>
              ...
              {member.id !== currentUser.id && <button className="icon-button member-delete-button" type="button" aria-label={`${member.displayName}をメンバーから削除`} onClick={() => { setMemberError(""); setRemovingMember(member); }}><Trash2 /></button>}
            </article>)}
            {memberError && <p className="form-error" role="alert">{memberError}</p>}
          </div>}
        </section>
        <section className="family-section invite-section">
          <h2>招待URL</h2><p>招待するメンバーの権限を選び、参加用URLを発行します。</p>
          {!inviteUrl ? <div className="form-stack"><fieldset className="role-guide">
            ...
            <label className={inviteRole === "owner" ? "selected" : ""}><input type="radio" name="invite-role" value="owner" checked={inviteRole === "owner"} onChange={() => setInviteRole("owner")} /><div><strong>管理者</strong><p>投稿・閲覧・コメント、イベント管理、メンバーの招待・削除、すべての投稿の削除</p></div></label>
    ...
    {removingMember && <div className="modal-backdrop" onClick={() => { if (!removing) setRemovingMember(null); }}>
      <section className="confirmation-modal" role="dialog" aria-modal="true" aria-labelledby="remove-member-title" onClick={(event) => event.stopPropagation()}>
        <h2 id="remove-member-title">{removingMember.displayName}を削除する？</h2>
        <p>このメンバーはログインできなくなる。これまでの投稿やコメントは残る。</p>
        {memberError && <p className="form-error" role="alert">{memberError}</p>}
        <div><button className="outline-button" type="button" disabled={removing} onClick={() => setRemovingMember(null)}>キャンセル</button><button className="danger-confirm-button" type="button" disabled={removing} onClick={() => void removeMember()}>{removing ? "削除中…" : "削除する"}</button></div>
      </section>
    </div>}
  </>;
}
