...
export function PostCreatePage() {
  ...
  const requestUploads = async (postId: string) => {
    const response = await api<{ media: UploadTarget[] }>(`/posts/${postId}/media/upload-urls`, {
      method: "POST",
      body: JSON.stringify({
    ...
    await uploadEntries(
      postId,
      prepared.map((item, index) => ({ item, index, target: response.media[index] })),
    );
  };
  ...
  const retryUploadSetup = async () => {
    ...
    setProgress(0);
    try {
      await requestUploads(draftPostId);
    } catch (reason) {
      setError((reason as Error).message);
      setBusy(false);
    }
      ...
      <main className="form-page page-content">
        <form onSubmit={submit} className="form-stack">
            ...
            <div className="selected-photos">
              {files.map((item, index) => (
                ...
                >
                  ...
                  {!draftPostId && (
                    <button type="button" onClick={() => removeFile(index)} aria-label={`${item.file.name}を外す`}>
                      <X />
                    </button>
                  )}
                </div>
          ...
          {busy ? null : files.some((item) => item.status === "failed") ? (
            ...
            <button className="primary-button wide" disabled={preparing || files.length === 0}>
              投稿
            </button>
          )}
        </form>
