...
export async function processNotificationBatches(
  ...
  `).bind(now.toISOString()).all<NotificationBatch>();
  ...
  const recipients = await env.DB.prepare(`
    SELECT id, line_user_id
      FROM users
     WHERE notification_enabled = 1 AND line_user_id IS NOT NULL
     ORDER BY id
  ...
  for (const batch of due.results) {
    ...
    const results = await Promise.allSettled(recipients.results.map(async (recipient) => {
      await send({
...
export async function notificationRetryKey(batchId: string, userId: string): Promise<string> {
  const digest = new Uint8Array(await crypto.subtle.digest("SHA-256", new TextEncoder().encode(`${batchId}:${userId}`))).slice(0, 16);
